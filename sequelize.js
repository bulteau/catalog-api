...
*/
async function requestProximity(id, l, a, b, limit = 10) {
  return await sequelize.query('SELECT * FROM products ' +
  'WHERE id != ?' +
  'ORDER BY (sqrt(power(dominant_color_l - ?, 2) + power(dominant_color_a - ?, 2) + power(dominant_color_b - ?, 2))) ' +
  'ASC LIMIT ?',
    { replacements: [id, l, a, b, limit], type: sequelize.QueryTypes.SELECT }
  );
}
